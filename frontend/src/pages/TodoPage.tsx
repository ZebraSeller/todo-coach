...
export const TodoPage = () => {
  ...
  const openSummary = async (date: string) => {
    ...
    try {
      ...
      setSummaryText(e instanceof Error ? e.message : 'Failed to summarize')
    } finally {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-400">
            Tasks are grouped by date. Use the summarize button to get AI guidance for the day.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          ...
          >
            ...
            <div className="rounded-xl bg-slate-900/80 border border-slate-800 px-3.5 py-3 text-sm text-slate-200 max-h-60 overflow-y-auto">
              {summaryLoading && <p className="text-slate-400 text-sm">Calling Vertex AI…</p>}
            ...
            <p className="text-[11px] text-slate-500">
              This summary is generated by {"Gemini"} using your tasks for the selected date.
            </p>
          </motion.div>
        </div>
      )}
